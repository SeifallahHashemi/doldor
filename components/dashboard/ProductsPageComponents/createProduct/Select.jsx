...
const Select = ({ options, selectName, className, onSelect, selectedItem, name, required }) => {
  ...
  return (
    <div className={`w-full bg-transparent border-[1px] border-slate-200 h-12 rounded-md relative common text-slate-200 cursor-pointer ${className} ${required ? "before:content-['_*'] before:absolute before:top-1/2 before:-translate-y-1/3 before:-left-6 before:text-red-900 before:text-4xl" : ""}`} onClick={onToggleSelectHandler}>
      <span ref={spanRef} className={cn("absolute top-1/2 right-4 -translate-y-1/2 transition-all text-xl px-4 text-slate-900 duration-[100ms] pointer-events-none", (toggleSelectOptions || selectedItem) && "top-0 text-sm text-gray-900")}
        style={{
          backgroundColor: bgColor
        }}
      >{selectName || "Select"}</span>
      <IoIosArrowDown className={cn("absolute top-1/2 left-1 -translate-y-1/2 transition-all text-slate-900", toggleSelectOptions && "rotate-180")} size={"2rem"} />
      <span className="absolute top-1/2 right-4 -translate-y-1/2 transition-all text-xl px-4 text-slate-900 duration-[100ms] pointer-events-none">{selectedItem || null}</span>
      {toggleSelectOptions && <ul className="bg-white border absolute top-full left-0 right-0 mt-4 text-slate-900 flex flex-col gap-1 rounded-md z-50 max-h-52 overflow-y-auto overflow-x-clip"
