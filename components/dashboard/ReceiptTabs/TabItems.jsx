...
const TabItems = ({title, id}) => {
    ...
    return (
        <li className={`${Number(receiptId) === id ? `${classes.active} text-white bg-[#f43f5e]` : "border-[0.1rem]"}  group px-4 rounded-t-lg relative  transition-all`}>
            <Link href={`/dashboard/receipt/${id}`} className={"flex flex-row-reverse flex-nowrap gap-8 px-6 py-1 font-iranYekan font-bold text-lg"}>
                ...
                <button className={"rotate-45 text-center content-center px-2 hover:text-doldor_orange border-none outline-none"}
                        onClick={() => {
                            ...
                            router.push(`/dashboard/receipt/${lastCustomerId}`);
                        }}>
                    <FaPlus/>
                </button>
            </Link>
